...
namespace MirTools.FileStructures
{
    public class WeMadeLibrary
    {
        ...
        public WeMadeLibrary(string name, bool InitializeImages = true)
        {
        ...
        public void Initialize(bool InitializeImages = true)
        {
            ...
            if (InitializeImages == true) InitializeLibraryImages(null);
        }

        public void InitializeLibraryImages(int[] Indexes)
        {
            if (Indexes == null)
        ...
        public void ToImageArray()
        {
            ...
            MLibraryV2 library = new MLibraryV2(null) { Images = new List<MLibraryV2.MImage>(), IndexList = new List<int>(), Count = Images.Length };
            //library.Save();
            ...
            try
            {
                Parallel.For(0, Images.Length, options, i =>
                {
                    WeMadeImage image = Images[i];
                    if (image.HasMask)
                        library.Images[0] = new MLibraryV2.MImage(image.Image, image.MaskImage) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.boHasShadow ? (byte)1 : (byte)0, MaskX = image.X, MaskY = image.Y };
                    else
                        library.Images[0] = new MLibraryV2.MImage(image.Image) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.boHasShadow ? (byte)1 : (byte)0 };
        ...
        public class WeMadeImage
        {
            ...
            public WeMadeImage(BinaryReader reader, byte nType)
            {
                ...
                if (nType == 1)
                {
                    bo16bit = (reader.ReadByte() == 5 ? true : false);// t :f
                    reader.ReadBytes(3);
                }
